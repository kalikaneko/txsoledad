...
class SoledadBackend(CommonBackend):
    ...
    """
    A U1DB backend implementation, with modifications for the Soledad Protocol
    that diverges from the original U1DB spec.
    """

    BATCH_SUPPORT = False

    def __init__(self, database, replica_uid=None):
        # TODO define the interface for "database" objects.
        """
...
class SoledadSyncTarget(CommonSyncTarget):
    ...
    def get_sync_info(self, source_replica_uid):
        ...
        d1 = self._db._get_replica_gen_and_trans_id(source_replica_uid)
        d2 = self._db._get_generation_info()
        d3 = self._db.get_replica_uid()
